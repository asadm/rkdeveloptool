...
I'll describe the steps for building this demo in two parts. In this blog post, I'll describe how I ported libusb itself, and what tricks might be necessary to port other popular libraries to Fugu APIs. In the [second post](https://web.dev/articles/porting-gphoto2-to-the-web), I'll go into details on porting and integrating gPhoto2 itself.
...
[The demo](https://web-gphoto2.rreverser.com/) running on a laptop connected to a Sony camera.
...
On Linux, you will likely need to [set custom permissions](https://developer.chrome.com/articles/build-for-webusb#linux) to allow access to your DSLR via WebUSB, although this depends on your distribution.
...
![Android phone connected to a Canon camera via a USB-C cable.](https://web.dev/static/articles/porting-libusb-to-webusb/image/android-phone-connected-5bc06721b5e43.jpg)

[The same demo](https://web-gphoto2.rreverser.com/) running on an Android phone. Picture by [Surma](https://twitter.com/DasSurma).

For a more in-depth guide on cross-platform usage of WebUSB, see the ["Platform-specific considerations" section of "Building a device for WebUSB"](https://developer.chrome.com/articles/build-for-webusb#platform-specific-considerations).
...
As mentioned [above](#build_system_and_the_first_test), the port depends on a few Emscripten features that currently need to be enabled at the linking stage of the application. If you want to use this libusb port in your own application, here's what you'll need to do:
